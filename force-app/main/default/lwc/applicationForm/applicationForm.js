...
import triggerScoring        from '@salesforce/apex/ApplicationController.triggerScoring';
...
export default class ApplicationForm extends NavigationMixin(LightningElement) {
    ...
    handleAdresse(event)  { this.adresse   = event.target.value; }
    handleCodePostale(event) { this.codePostale = event.target.value; }
    ...
    handleSubmit(event) {
        ...
        .then(() => {
            // Déclencher le scoring IA explicitement (filet de sécurité si le trigger CDL n'a pas fire)
            console.log('🤖 Déclenchement scoring IA pour Lead: ' + this._currentLeadId);
            return triggerScoring({ leadId: this._currentLeadId }).catch(err => {
                console.warn('⚠️ triggerScoring non bloquant:', err);
            });
        })
        .then(() => {
